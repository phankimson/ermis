'use strict'
const Data = use('App/Model/DiscountCoupon')  // EDIT
const Antl = use('Antl')
...
class DiscountCouponController{
  constructor () {
      this.type = ""  // EDIT
      this.key = "discount-coupon"  // EDIT
      this.menu = "pos_discount_coupon"  // EDIT
      this.download = "DiscountCoupon.xlsx"  // EDIT
      this.room = "discount-coupon"
    }
  * show (request, response){
      const title = Antl.formatMessage('discount_coupon.title')  // EDIT
      const data = yield Data.query().orderBy('id', 'desc').fetch()
      const stock = yield Inventory.query().where('active',1).orderBy('id', 'desc').fetch()
      const show = yield response.view('pos/pages/discount_coupon', {key : this.key, room : this.room  ,title: title , stock:stock.toJSON() , data: data.toJSON()})  // EDIT
      response.send(show)
  }
