...
class IssueInventoryGeneralController{
  constructor () {
      this.type = 2  // EDIT Receipt = 1 , Issue = 2 , Transfer = 3
      this.key = "issue-inventory-general"  // EDIT
      this.menu = "pos_issue_inventory"
      this.print = 'PXK%'
    }
  * show (request, response){
      const title = Antl.formatMessage('issue_inventory_general.title')  // EDIT
      ...
      const show = yield response.view('pos/pages/issue_inventory_general', {key : this.key ,title: title , data: data.toJSON() , end_date : end_date , start_date : start_date , print : print.toJSON() })  // EDIT
      response.send(show)
  }
  ...
  * prints (request, response){
    ...
    response.json({ status: false , error : true , message: Antl.formatMessage('messages.no_data')})
      }
  }
}
module.exports = IssueInventoryGeneralController
