...
class WholesaleVoucherController{
  constructor () {
      this.type = 4  // EDIT Wholesale = 4 ; Sale = 5
      this.key = "wholesale-voucher"  // EDIT
      this.menu = "pos_wholesale_voucher"
      this.voucher = 'WHOLESALE_%'
      this.print = 'BS%'
    }

    * show (request, response){
        const title = Antl.formatMessage('wholesale_voucher.title')  // EDIT
        ...
        .select("goods_size.id","goods_size.id as item_id","marial_goods.name as item_name","goods_size.barcode as code","marial_goods.name as name","unit.name as unit","goods_size.price","goods_size.purchase_price","size.name as size","discount.discount","discount.discount_percent").fetch()
        const print = yield PrintTemplate.query().where('code', 'LIKE', this.print ).fetch()
        const show = yield response.view('pos/pages/wholesale_voucher', {key : this.key ,title: title , voucher : voucher, item : item.toJSON() , print : print.toJSON()})  // EDIT
        response.send(show)
    }
    ...
    * scan (request, response){
      ...
      try{
        ...
        .innerJoin('size', 'size.id', 'goods_size.size')
        .leftJoin('discount', 'discount.id', 'marial_goods.discount')
        .select("goods_size.id","goods_size.id as item_id","marial_goods.name as item_name","goods_size.barcode as code","marial_goods.name as name","unit.name as unit","goods_size.price","goods_size.purchase_price","size.name as size","discount.discount","discount.discount_percent").first()
      }
        if(arr){
            response.json({ status: true  , data : arr })
        }else{
              ...
              if(closing[0].total == 0){
                ...
                if(data.id){
                 ...
                 //
                  ...
                  general.discount_percent =  data.discount_percent || 0
                  general.discount = data.discount || 0
                  ...
                  for(var d of data.detail){
                    ...
                    detail.amount = (d.price * d.quantity)*(1-(d.discount_percent/100))-d.discount
                    detail.discount_percent = d.discount_percent
                    detail.discount = d.discount
                    detail.total_discount = (d.price * d.quantity)*(d.discount_percent/100)+d.discount
...
module.exports = WholesaleVoucherController
