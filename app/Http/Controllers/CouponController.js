'use strict'
const Data = use('App/Model/Coupon')  // EDIT
const Antl = use('Antl')
...
class CouponController{
  constructor () {
      this.type = ""  // EDIT
      this.key = "coupon"  // EDIT
      this.menu = "pos_coupon"  // EDIT
      this.download = "Coupon.xlsx"  // EDIT
      this.room = "coupon"
    }
  * show (request, response){
      const title = Antl.formatMessage('coupon.title')  // EDIT
      const data = yield Data.query().orderBy('id', 'desc').fetch()
      const stock = yield Inventory.query().where('active',1).orderBy('id', 'desc').fetch()
      const show = yield response.view('pos/pages/coupon', {key : this.key, room : this.room  ,title: title , stock:stock.toJSON() , data: data.toJSON()})  // EDIT
      response.send(show)
  }
