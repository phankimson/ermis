'use strict'
const Data = use('App/Model/Discount')  // EDIT
const Antl = use('Antl')
...
class DiscountController{
  constructor () {
      this.type = ""  // EDIT
      this.key = "discount"  // EDIT
      this.menu = "pos_discount"  // EDIT
      this.download = "Discount.xlsx"  // EDIT
      this.room = "discount"
    }
  * show (request, response){
      const title = Antl.formatMessage('discount.title')  // EDIT
      const data = yield Data.query().orderBy('id', 'desc').fetch()
      const stock = yield Inventory.query().where('active',1).orderBy('id', 'desc').fetch()
      const show = yield response.view('pos/pages/discount', {key : this.key, room : this.room  ,title: title , stock:stock.toJSON() , data: data.toJSON()})  // EDIT
      response.send(show)
  }
    ...
    if(data){
      ...
      var action = 0
        ...
        if(permission.a || permission.e){
          ...
          result.discount_percent = data.discount_percent?data.discount_percent: 0
          result.inventory = data.inventory
          result.accumulative = data.accumulative
