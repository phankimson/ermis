'use strict'
const Data = use('App/Model/CustomerCoupon')  // EDIT
const Antl = use('Antl')
...
class CustomerCouponController{
  constructor () {
      this.type = ""  // EDIT
      this.key = "customer-coupon"  // EDIT
      this.menu = "pos_customer_coupon"  // EDIT
      this.download = "CustomerCoupon.xlsx"  // EDIT
      this.room = "customer-coupon"
    }
  * show (request, response){
      const title = Antl.formatMessage('customer_coupon.title')  // EDIT
      const data = yield Data.query().orderBy('id', 'desc').fetch()
      const stock = yield Inventory.query().where('active',1).orderBy('id', 'desc').fetch()
      const show = yield response.view('pos/pages/customer_coupon', {key : this.key, room : this.room  ,title: title , stock:stock.toJSON() , data: data.toJSON()})  // EDIT
      response.send(show)
  }
    ...
    if(data){
      ...
      var action = 0
        ...
        if(permission.a || permission.e){
          ...
          result.discount_percent = data.discount_percent
          result.cumulative = data.cumulative
...
module.exports = CustomerCouponController
